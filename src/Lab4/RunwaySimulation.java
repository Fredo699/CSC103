package Lab4;
/* CSC103 RunwaySimulation Fred Frey & TImothy Haskihns 4/20/16 */
...
class RunwaySimulation{
   ...
   public static void runwaySimulate(int landing_time, int takeoff_time, int average_takeoff_time, int average_landing_time, int max_landtime, int total_time){
	  ...
	  LinkedStack<Integer> crash_times = new LinkedStack<Integer>();
      ...
      for(int min = 1; min <= total_time; min++){
    	  ...
    	  if(time_to_next_landing == 0){
    		if (!rw.isBusy()){
    			...
    			next_lander = new Plane(min, 'L');
    			landed.add(next_lander);
    			System.out.println("Plane #" + next_lander.getPlaneNo() + " successfully landing.");
    		}
    		else{
    			next_lander = new Plane(min, 'L');
    			landings.add(new Plane(min, 'L')); // Add a new plane to the queue.
    			System.out.println("Plane #" + next_lander.getPlaneNo() + " arrived to land.");
    		}
    		number_to_land += 1;
    	  ...
    	  if(time_to_next_takeoff == 0){
      		if (!rw.isBusy() && landings.isEmpty()){
          		...
          		next_lander = new Plane(min, 'T');
          		tookoff.add(next_lander);
          		System.out.println("Plane #" + next_lander.getPlaneNo() + " successfully took off.");
      		}
      		else{
      			next_lander = new Plane(min, 'T');
      			takeoffs.add(next_lander); // Add a new plane to the queue.
      			System.out.println("Plane #" + next_lander.getPlaneNo() + " arrived to take off.");
      		}
      		number_to_takeoff += 1;
    	  ...
    	  while(!landings.isEmpty()){
    		  ...
    		  if ((min - next_lander.getTime()) > max_landtime){
    			  ...
    			  crash_times.push(min);
    			  System.out.println("Plane #" + next_lander.getPlaneNo() + " crashed.");
    		  }
    		  else
      ...
      while(!crashed.isEmpty()){
    	  next_lander = crashed.pop();
    	  crash_time = crash_times.pop();
    	  System.out.println("Plane #" + next_lander.getPlaneNo() + " crashed at " + crash_time + " minutes.");
      }
   }
}